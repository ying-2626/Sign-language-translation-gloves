...
import com.example.yyma.fingerinput.Sentence;
import com.example.yyma.fingerinput.WordBank;
...
public class ReceiveActivity extends Activity {
    ...
    private String sdcard = getExternalStorageDirectory().getAbsolutePath();
        ...
        if (lastOne == '1' || lastOne == '2' || lastOne == '3' || lastOne == '4' || lastOne == '5'){
            try {
                ...
                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.R) {
                    // Android 11及以上使用应用专用目录
                    File appDir = new File(getExternalFilesDir(null), "fingerinput");
                    audioFilePath = new File(appDir, path + ".wav").getAbsolutePath();
                } else {
                    // Android 11以下使用原有路径
                    audioFilePath = sdcard + "/fingerinput/" + path + ".wav";
                }
